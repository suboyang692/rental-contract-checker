"""评测脚本：标注集 vs 规则命中 → 精确率 / 召回率（纯规则，不调 LLM）

用法：python eval/evaluate.py
说明：标注集是 3 份样本合同的"预期命中"，用于回归验证规则引擎的准确度。
"""
import json
import os
import sys
...
def evaluate_file(file_name: str, annotations: list[dict]) -> dict:
    ...
    return {
        ...
        "unmatched_predictions": [
            f for i, f in enumerate(findings) if i not in matched_ids
        ],
    }
...
def main() -> None:
    ...
    for file_name, anns in sorted(annotations.items()):
        ...
        for f in r["unmatched_predictions"]:
            print(f"    ⚠️ 未匹配标注的命中: [{f['rule_id']}] {f['matched'][:50]}")
    print("-" * 62)
